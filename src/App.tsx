...
function CharactersDressUpView({ customClothes, setCustomClothes, characterBaseUrl, setCharacterBaseUrl, onDone }: any) {
  ...
  const handleUploadClothing = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      ...
      setCustomClothes([...customClothes, b64]);
    }
  };
        ...
        <div className="w-1/3 bg-wood-light/40 border border-wood-dark/20 rounded-lg p-4 custom-scrollbar overflow-y-auto z-10 flex flex-col">
           ...
           <div className="grid grid-cols-2 gap-3 flex-1 content-start">
              {customClothes.map((url: string, i: number) => (
                <div key={i} className="relative group aspect-square bg-paper border border-wood-dark/10 rounded-md flex justify-center items-center cursor-pointer hover:bg-white/90 hover:scale-105 shadow-sm transform transition-all">
                   <div onClick={() => handleAddClothes(url)} className="w-full h-full flex justify-center items-center overflow-hidden p-1">
                     <img src={url} alt={`Clothing ${i}`} className="w-full h-full object-contain drop-shadow-md pointer-events-none" />
                   </div>
                   <button 
                     onClick={(e) => { e.stopPropagation(); setCustomClothes((prev: string[]) => prev.filter((_, idx) => idx !== i)); }}
                     className="absolute top-1 right-1 bg-red-400 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity z-20 hover:bg-red-500 shadow-sm"
                   >
                     <X className="w-3 h-3" />
                   </button>
                </div>
...
function DecorationsMenu({ onAdd, customDecals, setCustomDecals }: any) {
  const handleUploadDecal = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      const b64 = await fileToBase64(e.target.files[0]);
      setCustomDecals([...customDecals, b64]);
    }
  };
    ...
    <div className="flex flex-col items-center min-h-[500px]">
      ...
      <div className="grid grid-cols-4 gap-6 w-full px-8">
        {customDecals.map((url: string, i: number) => (
          <div 
            key={i} 
            className="relative group aspect-square bg-paper border-2 border-dashed border-wood-dark/20 rounded-xl flex items-center justify-center hover:scale-105 hover:bg-white transition-all shadow-sm transform"
          >
            <button 
              onClick={() => onAdd(url)}
              className="w-full h-full p-2 overflow-hidden flex justify-center items-center rounded-xl"
            >
               <img src={url} alt={`Sticker ${i}`} className="w-full h-full object-contain drop-shadow-sm pointer-events-none" />
            </button>
             <button 
               onClick={(e) => { e.stopPropagation(); setCustomDecals((prev: string[]) => prev.filter((_, idx) => idx !== i)); }}
               className="absolute -top-2 -right-2 bg-red-400 text-white rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-20 hover:bg-red-500 shadow-sm"
             >
               <X className="w-4 h-4" />
             </button>
          </div>
...
function GoalsTrackerView() {
  const [goals, setGoals] = useLocalForage('corner_goals', [
    { id: 1, title: "Create my ultimate little corner", details: "Decorate my personal page and add my daily habits", completed: false }
  ]);
